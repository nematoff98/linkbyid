...
<template>
  <section id="how-it-works" class="w-full relative py-12">
    <div class="text-center max-w-2xl mx-auto mb-16 space-y-4">
      <h2 class="text-indigo-400 font-semibold tracking-wide uppercase text-sm">How it works</h2>
      <h3 class="text-3xl md:text-5xl font-bold text-white tracking-tight">Three steps to simplicity</h3>
      <p class="text-neutral-400 text-lg">Stop relying on deep links in descriptions. Consolidate your strategy into an intuitive search experience.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6 relative">
      ...
      >
        <div class="w-24 h-24 bg-neutral-900 border border-neutral-800 rounded-3xl flex items-center justify-center mb-6 shadow-xl relative transition-transform duration-300 group-hover:-translate-y-2">
           ...
           <div class="w-12 h-12 rounded-2xl bg-neutral-950 flex items-center justify-center" v-html="step.icon"></div>
           <div class="absolute -bottom-3 -right-3 w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white text-xs font-bold ring-4 ring-neutral-950 shadow-sm shadow-indigo-500/50">{{ step.number }}</div>
        </div>
        
        <h4 class="text-xl font-bold text-white mb-3">{{ step.title }}</h4>
        <p class="text-neutral-400 leading-relaxed max-w-[280px]">
          {{ step.description }}
        </p>
      </div>
    </div>
