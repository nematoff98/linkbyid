    ...
    <div class="group relative overflow-hidden rounded-2xl border border-neutral-200 bg-white transition-all duration-200 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-400/20 dark:border-white/12 dark:bg-neutral-900">
      <div class="relative flex h-12 items-center sm:h-[56px]">
        <div class="shrink-0 pl-3 pr-2 text-neutral-500 sm:pl-4 dark:text-neutral-400">
          <svg v-if="isSearching" class="w-4 h-4 animate-spin sm:w-5 sm:h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
          <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.3"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
        </div>
        ...
        <button
          v-if="query"
          @click="clearQuery"
          class="mr-2 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-neutral-200 bg-neutral-100 text-neutral-600 transition hover:bg-neutral-200 hover:text-neutral-800 focus:outline-none focus:ring-2 focus:ring-indigo-400/70 sm:mr-3 sm:h-8 sm:w-8 dark:border-white/14 dark:bg-white/8 dark:text-neutral-300 dark:hover:bg-white/14 dark:hover:text-white"
